...
int manager::getCapitial(){ return capital; }; 
void manager::updateCapital(double amount){ capital += amount; } 


int manager::getStockCapital(){ return stock_capital; }
...
void manager::list_attributes(bool list_name){
    std::cout << '\n';
    if(list_name){ type_chars("Name: " + name); }
